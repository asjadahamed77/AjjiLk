...
const Requests = () => {
  ...
  const handleSubmit = async (phoneId) => {
    ...
    if (selectedStatus === 'approve') {
      try {
        await axios.post(backendUrl+'/api/phone/approve', { phoneId });
        toast.success("Phone Approved to Display")
        // Optionally refetch the phones or update state after approval
      } catch (error) {
      ...
      try {
        ...
        // Optionally refetch the phones or update state after removal
      } catch (error) {
