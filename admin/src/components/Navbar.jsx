...
const Navbar = ({setToken}) => {
  ...
  return (
    <div className=' fixed top-0 right-0 left-0 flex justify-between px-8 sm:py-2 py-3 shadow-md bg-white'>
      ...
      <button onClick={logout} className="bg-transparent hover:text-lightColor hover:bg-mainColor ml-4 text-mainColor border-2 border-mainColor font-semibold px-3 rounded-full py-0.5">Logout</button>
    </div>
  )
}
