            ...
            <div className='pl-10 pt-10 w-[250px]'>
                <div className='border border-backgroundColor p-4'>
                    <div className='flex gap-2 items-center'>
                        ...
                        <div
                            className={`text-mainColor text-lg cursor-pointer ${showCategory ? '' : '-rotate-90'} sm:hidden`}
                            onClick={toggleCategory}
                        >
                            <FaArrowAltCircleDown />
                        </div>
                    </div>
                    <div className={`${showCategory ? 'flex' : 'hidden'} sm:flex mt-3 ml-3 flex-col gap-3`}>
    ...
    <p className="font-medium text-base">Realme</p>
            ...
            <div className='flex-1 pt-6 px-6'>
                <div className='flex justify-between'>
                    ...
                    <select
                        className='md:mr-8 lg:mr-10 xl:mr-16 mr-4 text-sm sm:text-base rounded-lg bg-lightColor border border-mainColor text-mainColor'
                    >
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4'>
                    {
                      phones.map((phone,index)=>(
                        <Phone key={index} id={phone._id} phoneImages={phone.phoneImages} phoneName={phone.phoneName} phoneBrand={phone.phoneBrand} phonePrice={phone.phonePrice}  />
                      ))
                    }
                </div>
            </div>
